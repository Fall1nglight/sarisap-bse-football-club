...
.venue-teaser { position: relative; min-height: 42rem; display: grid; align-items: center; overflow: hidden; }.venue-teaser > :deep(img) { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }.venue-teaser::after { position: absolute; inset: 0; background: linear-gradient(90deg,rgba(12,19,15,.65),transparent 70%); content:''; }.venue-teaser__card { position: relative; z-index: 1; width: min(100% - 2 * var(--page-gutter), var(--container)); padding: clamp(2rem,5vw,4rem); border-radius: .75rem; background: var(--color-canvas); }.venue-teaser__card h2 { margin-bottom: 1rem; }.venue-teaser__card p:not(.eyebrow) { margin-bottom: 2rem; color: var(--color-muted); }
